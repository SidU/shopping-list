...
export function ItemSuggestions({
  ...
  return (
    <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-background border rounded-lg shadow-lg overflow-hidden max-h-64 overflow-y-auto">
      {showAddNew && (
        <button
          type="button"
          onClick={onAddNew}
          className="w-full px-4 py-3 text-left hover:bg-accent flex items-center gap-2 border-b"
        >
          <span className="text-primary font-medium">Add "{query.trim()}"</span>
          <span className="text-muted-foreground text-sm">as new item</span>
        </button>
      )}
      ...
      {suggestions.map((item) => (
        <button
          ...
          className="w-full px-4 py-3 text-left hover:bg-accent flex items-center justify-between gap-2"
        >
